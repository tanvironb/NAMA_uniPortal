...
const normalize = (str: string | null): string => {
  ...
  return str
    ...
    .replace(/&/g, 'and'); // Then convert all "&" to "and" for consistency
};
...
const fuzzyMatch = (value: string | null, target: string): boolean => {
  ...
  for (const [canonical, variations] of Object.entries(degreeVariations)) {
    ...
    const valueMatch = allVariants.some(v => normalizedValue.includes(v) || v.includes(normalizedValue));
    const targetMatch = allVariants.some(v => normalizedTarget.includes(v) || v.includes(normalizedTarget));
    
    if (valueMatch && targetMatch) return true;
  }
  
  // Handle field variations (engineering, etc.)
...
export function UniversityCoursesModal({
  ...
  const { filteredCourses, hasExactMatches } = useMemo(() => {
    ...
    // Filter by level and field using fuzzy matching
    ...
    // Remove duplicates by (course_title, duration, scholarship_percentage)
                  ...
                  >
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        ...
                        {/* Meta Pills - 3 column on desktop, 2 rows on mobile */}
                        <div className="flex flex-wrap gap-2">
                          ...
                          >
                            ...
                            <span className="text-muted-foreground">{course.duration || "Not specified"}</span>
                          </div>
                          ...
                          >
                            ...
                            <span className="text-muted-foreground">{formatScholarshipPercentage(course.scholarship_percentage)}</span>
                          </div>
                        </div>
                      </div>
